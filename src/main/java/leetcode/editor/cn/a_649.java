...
public class a_649 {
    public static void main(String[] args) {
        a_649.Solution solution = new a_649().new Solution();
    ...
    class Solution {
        public String predictPartyVictory(String senate) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (c == 'R') dequeR.offer(i);
                else dequeD.offer(i);
            }
            while (!dequeD.isEmpty() && !dequeR.isEmpty()) {
                if (dequeD.peek() > dequeR.peek()) {
                    dequeD.poll();
                    dequeR.offer(dequeR.poll() + length);
                } else {
                    dequeR.poll();
                    dequeD.offer(dequeD.poll() + length);
                }
            }
